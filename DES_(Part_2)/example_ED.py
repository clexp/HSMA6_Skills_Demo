...
class g:
	patient_inter = 2
	mean_reception_time = 4
	mean_n_consult_time = 30
	mean_d_consult_time = 60
	mean_a_consult_time = 20
	number_of_reception = 4
	number_of_nurses = 4
	number_of_doctors = 2
	number_of_acp = 4
	sim_duration = 8 * 60
	number_of_runs = 100
	prob_seeing_doctor = 0.4
	prob_seeing_acp = 0.4
...
class Model:
	def __init__(self, run_number):
		...
		self.acp = simpy.Resource(self.env, capacity=g.number_of_acp)
		...
		self.results_df["Q Time ACP"] = [0.0]
		self.results_df["Time with ACP"] = [0.0]
	...
	def attend_clinic(self, patient):
		...
		elif random.uniform(0,1) < g.prob_seeing_acp:
			start_q_acp = self.env.now
			with self.doctor.request() as req:
				yield req
				end_q_acp = self.env.now
				patient.q_time_acp = end_q_acp - start_q_acp
				sampled_acp_act_time = random.expovariate(1.0 / g.mean_a_consult_time)
				self.results_df.at[patient.id, "Q Time Doctor"] = patient.q_time_acp
				self.results_df.at[patient.id, "Time with Doctor"] = sampled_acp_act_time
				yield self.env.timeout(sampled_acp_act_time)
	...
	def calculate_run_results(self):
		...
		self.mean_q_time_acp = self.results_df["Q Time ACP"].mean()
...
class Trial:
	def __init__(self):
		...
		self.df_trial_results["Mean Q Time ACP"] = [0.0]
